<template>
  <div class="p-6 bg-gray-50 min-h-screen">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl  text-gray-900">Historique des Validations (RAF)</h1>
        <p class="text-sm text-gray-500">Consultez l'historique global des clôtures de caisse et les écarts détectés.
        </p>
      </div>
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-for="row in history" :key="row.id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-6 py-4">
                <Badge variant="outline" class="bg-blue-50 text-blue-700  border-blue-100">{{ row.nom_caisse }}</Badge>
              </td>
...
const fetchHistory = async (page = 1) => {
  loading.value = true;
  try {
    const { data } = await axios.get(route('get.raf.validations.history'), {
      params: {
        ...filters,
        page
      }
