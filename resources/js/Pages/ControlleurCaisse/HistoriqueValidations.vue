<template>
  <div class="p-6 bg-gray-50 min-h-screen">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl  text-gray-900">Mes Validations de Caisse</h1>
        <p class="text-sm text-gray-500">Historique des sessions de caisse validées sur votre site.</p>
      </div>
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-else-if="history.length === 0">
              <td colspan="10" class="px-6 py-12 text-center text-gray-400">Aucun historique trouvé pour votre site.
              </td>
            </tr>
            <tr v-for="row in history" :key="row.id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-6 py-4  text-sm text-amber-700 ">
                {{ formatMoney(row.montant_controlleur) }} F
              </td>
              <td class="px-6 py-4 text-right">
                ...
                <div v-else-if="row.surplus > 0" class="flex flex-col items-end">
                  ...
                  <Badge class="bg-green-100 text-green-700 border-green-200 text-[8px] px-1 py-0 h-4 ">SURPLUS</Badge>
                </div>
...
const fetchHistory = async (page = 1) => {
  loading.value = true;
  try {
    const { data } = await axios.get(route('get.caisse.controller.validations.history'), {
      params: {
        ...filters,
        page
      }
